...
#include <core/log/logger.hpp> // 假设你有这个
#include <imgui.h>
#include <imgui_internal.h> // 必须包含，用于 SettingsHandler
#include <implot.h>
#include <algorithm>
#include <vector>
...
namespace april::ui
{
    ...
    {
        m_profiler = GlobalProfiler::getManager();
    ...
    auto ElementProfiler::add_entries(
        april::core::ProfilerTimeline::Snapshot const& snapshot,
    ...
    auto ElementProfiler::render_line_chart(View& view) -> void
    {
        ...
        for(size_t i = 0; i < m_frameNodes.size(); ++i)
        {
            ...
            if(!rootNode.child.empty())
            {
                ...
                // 2. Data Gathering & Stacking Logic (还原)
                ...
                for(size_t k = 0; k < node.child.size(); k++)
                {
                    ...
                    if(!child.timerInfo.gpu.times.empty())
                    {
                        ...
                        {
                            // Ring buffer access
                            uint32_t index = (child.timerInfo.gpu.index - child.timerInfo.numAveraged + j) % ProfilerTimeline::kMaxLastFrames;
                            gpuTimes[k][j] = float(child.timerInfo.gpu.times[index] / 1000.0);

                            // Stacking!
                            if(k > 0) gpuTimes[k][j] += gpuTimes[k-1][j];

                            gpuTimeSum += gpuTimes[k][j]; // Note: logic in source accumulates stacked sum for avg which implies top stack average
                        }
                ...
                {
                    uint32_t index = (node.timerInfo.cpu.index - node.timerInfo.numAveraged + j) % ProfilerTimeline::kMaxLastFrames;
                    cpuTimes[j] = float(node.timerInfo.cpu.times[index] / 1000.0);
                    avgCpuTime += cpuTimes[j];
                }

                // 3. Temporal Smoothing for Y-Axis (还原)
                float avgTime = 0.f;
    ...
    {
        m_vsync = vsync;
        // TODO: Call your Engine's Window VSync setter here.
        // Example: april::Application::get().getWindow().setVSync(vsync);
    }
